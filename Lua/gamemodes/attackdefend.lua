...
function gm:Start()
    ...
    teams[1] = {}
    teams[1].Name = "Защищающая команда"
    teams[1].Spawns = {}
    teams[1].Members = {}
    ...
    ---@diagnostic disable-next-line: missing-fields
    teams[2] = {}
    teams[2].Name = "Атакуящая команда"
    teams[2].Spawns = {}
    teams[2].Members = {}
...
function gm:Think()
    ...
    if self.LastDefendCountDown - self.DefendCountDown > max then
        for _, client in pairs(Client.ClientList) do
            Traitormod.SendChatMessage(client, "У команды защитников есть " .. math.ceil(self.DefendCountDown) .. " cекунд, чтоб защищать реактор!", Color.GreenYellow)
        end
    ...
    for _, team in pairs(self.Teams) do
        ...
        if team.CheckWinCondition() then
            self.Ending = true
            for _, client in pairs(Client.ClientList) do
                Traitormod.SendMessage(client, team.Name .. " выйграла игру!", "InfoFrameTabButton.Mission")
            end
