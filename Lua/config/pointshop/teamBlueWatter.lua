...
category.Identifier = "teamBlueWatter"

-- Проверка что это подходящий игрок:
-- 1) У него есть персонаж
-- 2) Этот персонаж жив(не мёртв)
-- 3) Этот персонаж находится во 1-й команде (Defenders Team)
---@param client Client
---@return boolean
category.CanAccess = function(client)
	return client.Character and not client.Character.IsDead and client.Character.TeamID == CharacterTeamType.Team1
end
...
category.Products = {
	{
		...
		Action = function(client)
			...
			Entity.Spawner.AddItemToSpawnQueue(helmet, client.Character.Inventory, nil, nil, function(item)
				item.set_InventoryIconColor(Color(0, 0, 122, 255))
				item.SpriteColor = Color(0, 0, 122, 255)
				item.Tags = "smallitem"
			...
			local armor = ItemPrefab.GetItemPrefab("combatdivingsuit")
			Entity.Spawner.AddItemToSpawnQueue(armor, client.Character.Inventory, nil, nil, function(item)
				...
				item.set_InventoryIconColor(Color(0, 0, 122, 255)) -- Цвет иконки в инвентаре
				item.SpriteColor = Color(0, 0, 122, 255) -- Цвет спрайта (предмета в игре)
				item.Tags = "smallitem"
			...
			local injector = ItemPrefab.GetItemPrefab("genesplicer") -- Шаблон генетики
			Entity.Spawner.AddItemToSpawnQueue(injector, client.Character.Inventory, nil, nil, function(item)
