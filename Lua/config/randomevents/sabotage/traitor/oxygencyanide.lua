...
event.Start = function()
    ...
    Traitormod.RoundEvents.SendEventMessage(Traitormod.Language.OxygenCyanide, "GameModeIcon.PVP", Color.Red)
    ...
    Timer.Wait(function()
        ...
        for _, character in pairs(Character.CharacterList) do
            if canBreatheMainSubOxygen(character) then
                applyAffliction(character, "cyanidepoisoning", 20)
            end
        end
    end, DELAY_MS)
